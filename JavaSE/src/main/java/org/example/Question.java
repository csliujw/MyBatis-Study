...
public class Question {
    ...
    public static class HeapNode implements Comparable {
        ...
        int s;// 根节点到当前节点的路径为x[0:s] 即 记录走了几步了
    ...
    public float tisProblem(int[] v) {
        ...
        while (enode != null && enode.s < n - 1) {
            ...
            if (enode.s == n - 2) {
                ...
                //再加两条边构成回路
                //所构成回路是否优于当前最优解
                if (graph[x[n - 2]][x[n - 1]] != 0
                        && graph[x[n - 1]][1] != 0
                        // PS：x中保存的是encode还需要搜索的结点
                        && (enode.currentCost + graph[x[n - 2]][x[n - 1]] + graph[x[n - 1]][1] < bestc)) {
                    //找到费用更小的回路
                ...
                for (int i = enode.s + 1; i < n; i++) {
                    ...
                    if (graph[x[enode.s]][x[i]] != 0) {
                        //可行儿子结点  graph[x[enode.s]][x[i]] 当前点到另一点（还需要搜索的点）的距离
                        float currentCost = enode.currentCost + graph[x[enode.s]][x[i]];
                        // 剩余点的最小费用和  + enode出边的最小值
                        float rcost = enode.rcost + minOut[x[enode.s]];
                        // 代价函数 = 当前已走过的cost + 剩余可能的最小值
                        // 如分为两部分 已遍历的  未遍历的。
                        float loss = currentCost + rcost;// 代价函数
                        // 代价函数与界的比较
                        if (loss < bestc || bestc == 0) {
                            //子树可能含有最优解，结点插入最小堆
                            int[] xx = new int[n];
                            for (int j = 0; j < n; j++)
                                xx[j] = x[j];
                            xx[enode.s + 1] = x[i];
                            xx[i] = x[enode.s + 1];
                            HeapNode node = new HeapNode(loss, currentCost, rcost, enode.s + 1, xx);
                            heap.add(node);
                            Collections.sort(heap);
                        }
                    }
...
//        int n = 4;
        ...
        Question question = new Question(weight);
        int[] v = new int[n + 1];
        System.err.println("最短回路长为：" + question.tisProblem(v));
